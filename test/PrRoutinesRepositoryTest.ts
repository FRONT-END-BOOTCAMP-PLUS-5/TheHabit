...
async function testCreate() {
  ...
  try {
    ...
    if (
      createdRoutine.id &&
      createdRoutine.routineTitle === testRoutine.routineTitle
    ) {
...
async function testFindByChallengeId() {
  ...
  try {
    ...
    const challengeRoutines = await routinesRepository.findByChallengeId(
      challengeId
    );
...
async function testUpdate() {
  ...
  try {
    ...
    // 루틴 업데이트
    const updatedRoutine = await routinesRepository.update(
      createdRoutineId,
      updateData
    );
