...
import { getChallengeProgress } from '@/public/utils/challengeUtils';
import {
  filterRoutineCompletionsByChallenge,
  filterRoutinesByChallenge,
  calculateCompletionRatio,
} from '@/public/utils/routineUtils';
...
const ChallengesAccordion: React.FC<ChallengesAccordionProps> = ({
  ...
  const completedRatio = challenge.id
    ? calculateCompletionRatio(routines, routineCompletions, challenge.id, selectedDate)
    : 0;
      ...
      >
        <div ref={contentRef}>
          <ChallengesAccordionContent
            challenge={challenge}
            routines={challenge.id ? filterRoutinesByChallenge(routines, challenge.id) : []}
            routineCompletions={
              challenge.id
                ? filterRoutineCompletionsByChallenge(
                    routineCompletions,
