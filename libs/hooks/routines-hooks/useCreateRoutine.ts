...
export const useCreateRoutine = () => {
  ...
  return useMutation<{ success: boolean; data?: ReadRoutineResponseDto; message?: string; error?: { code: string; message: string } }, Error, CreateRoutineRequestDto>({
    mutationFn: createRoutine,
    onSuccess: (data, variables) => {
      // 루틴 생성 성공 시 관련 캐시 무효화
      queryClient.invalidateQueries({ queryKey: ['routines', 'all'] });
      queryClient.invalidateQueries({ queryKey: ['routines', 'challenge', variables.challengeId] });
