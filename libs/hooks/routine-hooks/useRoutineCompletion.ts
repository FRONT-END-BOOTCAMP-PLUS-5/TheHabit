...
type RoutineCompletion = CreateRoutineCompletionResponseDto;
...
export function useRoutineCompletion({ 
  ...
  // 루틴 완료 여부 확인
  const isRoutineCompleted = useCallback((routineId: number) => {
    return completions.some((completion) => completion.routineId === routineId);
  }, [completions]);

  // 완료된 루틴의 completion 데이터 가져오기
  const getRoutineCompletion = useCallback((routineId: number) => {
  ...
  // 루틴 완료 삭제
  const deleteCompletion = useCallback(async (routineId: number) => {
    try {
